...
def train(args, no_of_items, load_model=False):
    ...
    # Load the weights for Fuji 16 to 8 bit converter (Use 2k epochs from GT only converter)
    ...
    target_trainset_1 = CanonTrainSet(args.target_input_dir, args.target_gt_dir)
    ...
    for epoch in range(args.start_epoch+1, args.num_epoch):
        ...
        if epoch % args.model_save_freq == 0:
            ...
            torch.save(state, args.checkpoint_dir + 'model_%d_%d_%d_sony_canon.pl' % (epoch, args.num_target_samples_1, 1))
...
def set_logger(args, logger_name, t_set, exp=0, task=0, track=None, level=logging.INFO):
    ...
    if exp == 1:
        fileHandler = logging.FileHandler(os.path.join(args.result_dir, f'log_Sony_canon_{args.num_target_samples_1}_{t_set}.txt'), mode='a')
    elif exp == 2:
        fileHandler = logging.FileHandler(os.path.join(args.result_dir, f'log_Sony_canon_{args.num_target_samples_2}_{t_set}.txt'), mode='a')
    elif logger_name == 'general':
        fileHandler = logging.FileHandler(os.path.join(args.result_dir, f'log_Sony_canon_general_{args.num_target_samples_1}.txt'), mode='a')
    elif exp == 3:
        fileHandler = logging.FileHandler(os.path.join(args.val_result_dir, f'loss_Sony_canon_{args.num_target_samples_1}_{task}.txt'), mode='a')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--target_input_dir', type=str, default='./dataset/Canon/short_final/')
    parser.add_argument('--target_gt_dir', type=str, default='./dataset/Canon/long_final/')
    ...
    parser.add_argument('--validation_gt_dir', type=str, default='./dataset/Canon/long_new/')
